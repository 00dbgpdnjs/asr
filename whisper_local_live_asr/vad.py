...
class Audio(object):
    ...
    def __init__(
        ...
        def proxy_callback(in_data, frame_count, time_info, status): # 1st: 음성 데이터
...
class VADAudio(Audio):
    ...
    def __init__(
        self, 
        aggressiveness: int = 3,  # 값이 높으면 애매할 때 사람이 아니라 판단
        device: int=None, 
        input_rate: int=None
    ):
    ...
    def frame_generator(self):
        ...
        if self.input_rate == self.rate_process:
            while True:
                yield self.read() # 버퍼에 있으면 계속 가져옴
        else:
    ...
    def vad_collector(self, padding_ms=300, ratio=0.75, frames=None):
        ...
        Determines voice activity by ratio of frames(ex. 0.1초 마다) in padding_ms. 
        Uses a buffer to include padding_ms prior to being triggered.
        ; 큐에서 음성만 뽑아옴
        Example: (frame, ..., frame, None, frame, ..., frame, None, ...) # frame: 사람 음성, None: 사람 목소리x(안가져옴)
                    |---utterence---|        |---utterence---|
        """
        if frames is None: 
        ...
        for frame in frames:
            ...
            is_speech = self.vad.is_speech(frame, self.sample_rate) # 사람 목소리인지 아닌지

            if not triggered: # frame, ..., frame (utterence)
                ring_buffer.append((frame, is_speech))
                num_voiced = len([f for f, speech in ring_buffer if speech])
                if num_voiced > ratio * ring_buffer.maxlen:
                    triggered = True
                    for f, s in ring_buffer: # 사람 소리이니까 버퍼에 있는 것을 보내줌 
                        yield f
                    ring_buffer.clear()
